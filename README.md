...
- **14 supported games:** `ApeStrong`, `Roulette`, `Baccarat`, `Jungle Plinko ✔︎`, `Cosmic Plinko ✔︎`, `Keno`, `Speed Keno`, `Dino Dough`, `Bubblegum Heist`, `Monkey Match`, `Bear-A-Dice`, `Primes ✔︎`, `Blackjack`, and `Video Poker ✔︎ / Gimboz Poker`
...
- **ABI-verified game metadata:** verified contracts are marked with `✔︎` in CLI output, help, JSON payloads, and docs; Jungle Plinko, Cosmic Plinko, Primes, and Video Poker use verified on-chain contract data
