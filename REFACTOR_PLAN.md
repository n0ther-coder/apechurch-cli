...
program.name('apechurch-cli').version(VERSION);
...
New commands:
- `apechurch-cli blackjack start 10` → starts game, shows state
- `apechurch-cli blackjack hit <gameId>` → hit action
- `apechurch-cli blackjack stand <gameId>` → stand action
- Or interactive mode: `apechurch-cli blackjack play 10` → prompts for actions
