...
`--solver-max-states` caps recursive player-state memoization. `--solver-timeout-ms` caps worker wall-clock latency. `best` defaults to `50000` states and `5000` ms for both `--auto` and manual `--solver`. `max` is the same exact solver with larger defaults: `150000` states and `30000` ms. These are operational guards, not contract rules and not mathematically derived precision parameters. The state cap was introduced after split-heavy spots such as `3,3` vs dealer `3` exhausted Node heap / CPU. When either guard trips, the CLI terminates or exits the worker path and falls back to `simple` basic strategy for that decision.

Manual `--solver [simple|best|max]` uses the same decision engines without submitting an action. Bare `--solver` defaults to `best`. For `best` and `max`, the CLI prints both the worker decision and the simple-strategy decision so operator-visible differences are explicit before a manual action is chosen.
