...
const MONKEY_MATCH_EXPECTED_RTP_BY_MODE = Object.freeze({
  1: 98.1475,
  2: 98.198,
});
...
const CALCULATION_MODEL_KENO = 'Exact hypergeometric EV for 10 draws from 40 without replacement using the published paytable.';
...
const CALCULATION_MODEL_MONKEY_MATCH = 'Exact weighted sum over the published mode probability table.';
...
const CALCULATION_MODEL_VIDEO_POKER = 'Exact weighted sum over the published final-hand odds and base paytable; progressive jackpot excluded unless configured.';
const CALCULATION_MODEL_COSMIC_PLINKO = 'Exact weighted sum over the published bucket probabilities for each public risk mode.';
const CALCULATION_MODEL_BLOCKS = 'Exact weighted sum over the published largest-cluster probability table for each mode.';
const CALCULATION_MODEL_PRIMES = 'Exact weighted sum over the published prime-hit and zero-jackpot probabilities by difficulty.';
...
const MONKEY_MATCH_CALCULATED_VARIANTS = Object.freeze([
  createCalculatedVariant({
    ...
    variantLabel: 'Mode 1 / Low Risk',
    value: MONKEY_MATCH_EXPECTED_RTP_BY_MODE[1],
  ...
  createCalculatedVariant({
    ...
    variantLabel: 'Mode 2 / Normal',
    value: MONKEY_MATCH_EXPECTED_RTP_BY_MODE[2],
    calculationModel: CALCULATION_MODEL_MONKEY_MATCH,
...
const COSMIC_PLINKO_CALCULATED_VARIANTS = Object.freeze([
  createCalculatedVariant({
    game: 'cosmic-plinko',
    variantKey: 'cosmic-plinko:mode:low',
    variantLabel: 'Low',
    value: 97.642,
  ...
  createCalculatedVariant({
    game: 'cosmic-plinko',
    variantKey: 'cosmic-plinko:mode:moderate',
  ...
  createCalculatedVariant({
    game: 'cosmic-plinko',
    variantKey: 'cosmic-plinko:mode:high',
    variantLabel: 'High',
    value: 99.29,
    calculationModel: CALCULATION_MODEL_COSMIC_PLINKO,
    maxPayout: fixedMaxPayout(250),
...
const PRIMES_CALCULATED_VARIANTS = Object.freeze([
  createCalculatedVariant({
    game: 'primes',
    variantKey: 'primes:mode:easy',
    variantLabel: 'Easy',
    value: 98,
    calculationModel: CALCULATION_MODEL_PRIMES,
    maxPayout: fixedJackpotMaxPayout(2.2),
  }),
  createCalculatedVariant({
    game: 'primes',
    variantKey: 'primes:mode:medium',
    variantLabel: 'Medium',
    value: 98,
    calculationModel: CALCULATION_MODEL_PRIMES,
    maxPayout: fixedJackpotMaxPayout(10.5),
  }),
  createCalculatedVariant({
    game: 'primes',
    variantKey: 'primes:mode:hard',
    variantLabel: 'Hard',
    value: 98,
    calculationModel: CALCULATION_MODEL_PRIMES,
    maxPayout: fixedJackpotMaxPayout(56),
  }),
  createCalculatedVariant({
    game: 'primes',
    ...
    value: 98.0353,
    calculationModel: CALCULATION_MODEL_PRIMES,
    maxPayout: fixedJackpotMaxPayout(500),
  }),
]);
...
const GAME_RTP_ENTRIES = [
  ...
  },
  {
    ...
    expected: null,
    reported: 98.42,
    aliases: ['jungle plinko', 'Jungle Plinko'],
  },
  {
    key: 'keno',
    expected: null,
  ...
  },
  {
    key: 'monkey-match',
    expected: null,
    calculated: fixedCalculated(Math.min(...Object.values(MONKEY_MATCH_EXPECTED_RTP_BY_MODE)), {
      calculationKind: 'exact',
      calculationModel: CALCULATION_MODEL_MONKEY_MATCH,
    }),
    reported: 97.34,
  ...
  },
  {
    key: 'cosmic-plinko',
    expected: null,
    calculated: fixedCalculated(97.642, {
      calculationKind: 'exact',
      calculationModel: CALCULATION_MODEL_COSMIC_PLINKO,
    }),
    reported: 97.32,
    aliases: ['cosmic plinko', 'Cosmic Plinko'],
  },
  {
    key: 'cult-quest',
    expected: null,
...
export function getConfiguredCalculatedVariantReference({ game, config = {} } = {}) {
  ...
  return resolvedVariant?.variantKey
...
export function getConfiguredGameMaxPayoutReference({ game, config = {} } = {}) {
  ...
  const exactVariant = resolvedVariant?.variantKey
...
export function resolveConfiguredGameVariant({ game, config = null } = {}) {
  ...
  switch (resolvedGameKey) {
    ...
    case 'monkey-match': {
      ...
      if (mode !== null) {
        ...
        variantLabel = modeName || (mode === 1 ? 'Mode 1 / Low Risk' : mode === 2 ? 'Mode 2 / Normal' : `Mode ${mode}`);
    ...
    case 'cosmic-plinko': {
      const mode = toFiniteInteger(rawConfig?.mode);
      const balls = toFiniteInteger(rawConfig?.balls);
      if (mode !== null || balls !== null) {
        variantKey = [
          resolvedGameKey,
          mode !== null ? `mode:${mode}` : null,
          balls !== null ? `balls:${balls}` : null,
        ].filter(Boolean).join(':');
        variantLabel = [
          mode !== null ? `Mode ${mode}` : null,
          balls !== null ? `${balls} balls` : null,
