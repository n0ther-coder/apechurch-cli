...
export async function playBearDice({
  ...
  const requestedRolls = resolveBearDiceRollsInput(rolls, runs);
...
function resolveBearDiceRollsInput(rolls, runs) {
  ...
  if (hasRolls && hasRuns) {
    ...
    if (Number.isFinite(parsedRolls) && Number.isFinite(parsedRuns) && parsedRolls !== parsedRuns) {
      throw new Error('Conflicting Bear-A-Dice roll count: --rolls and --runs must match, or only one may be provided.');
    }
  }
...
export function resolveBearDiceConfig(baseConfig = {}, opts, positionalConfig, strategyConfig, randomIntInclusive, options = {}) {
  ...
  const requestedRolls = resolveBearDiceRollsInput(opts.rolls, opts.runs);
  if (requestedRolls !== undefined) {
    config.rolls = parseInt(requestedRolls);
  } else if (positionalConfig.rolls !== undefined) {
    config.rolls = positionalConfig.rolls;
  } else if (config.rolls === undefined && preferGameDefault && gameEntry?.config?.rolls?.default !== undefined) {
    config.rolls = gameEntry.config.rolls.default;
