 ...
 * - 1 (Medium): Lose on 6,7,8 (44.44% per roll - risky!)
 * - 2 (Hard): Lose on 5,6,7,8,9 (66.67% per roll - very risky!)
 * - 3 (Expert): Lose on 4,5,6,7,8,9,10 (83.33% per roll - brutal)
 ...
 * - Expert (3) and Master (4) also expose 4-roll and 5-roll payout tables
 *
 * Strategy Note:
...
export const BEAR_DICE_GAME_INFO_ABI = [
  {
    type: 'function',
    name: 'getGameInfo',
 ...
 * @param {number} [params.survive] - Number of all-or-nothing survival rolls 1-5 (default: 1)
 * @param {number} [params.rolls] - Legacy internal alias for survival roll count
...
export async function playBearDice({
  ...
  const requestedRolls = resolveBearDiceSurviveInput(survive, rolls, runs);
  const numRolls = ensureIntRange(
    requestedRolls ?? gameEntry.config.rolls.default,
    'survive',
    gameEntry.config.rolls.min,
    gameEntry.config.rolls.max,
  );
  ...
  const config = {
    difficulty: diff,
    difficultyName: getGameOptionLabel(gameEntry, 'difficulty', diff, DIFFICULTY_NAMES[diff]),
    risk: diff,
    riskName: getGameOptionLabel(gameEntry, 'difficulty', diff, DIFFICULTY_NAMES[diff]),
    survive: numRolls,
  };
...
function resolveBearDiceSurviveInput(survive, rolls, runs) {
  ...
  if (hasRolls && hasRuns) {
    ...
    if (Number.isFinite(parsedRolls) && Number.isFinite(parsedRuns) && parsedRolls !== parsedRuns) {
      throw new Error('Conflicting Bear-A-Dice legacy survival count values.');
    }
  }

  return hasRolls ? rolls : hasRuns ? runs : undefined;
}
 ...
 *
 * Strategy logic:
 * - If risk is omitted, default to Easy (0)
 ...
 * @returns {Object} { difficulty: number, survive: number }
 */
export function resolveBearDiceConfig(baseConfig = {}, opts, positionalConfig, strategyConfig, randomIntInclusive, options = {}) {
  const config = { ...baseConfig };
  const gameEntry = options.gameEntry;
  ...
  } else if (config.difficulty === undefined && gameEntry?.config?.difficulty?.default !== undefined) {
  ...
  const requestedRolls = resolveBearDiceSurviveInput(opts.survive, opts.rolls, opts.runs);
  if (requestedRolls !== undefined) {
    config.survive = parseInt(requestedRolls);
  } else if (positionalConfig.rolls !== undefined) {
    config.survive = positionalConfig.rolls;
  } else if (config.survive === undefined && preferGameDefault && gameEntry?.config?.rolls?.default !== undefined) {
    config.survive = gameEntry.config.rolls.default;
  } else if (config.survive === undefined) {
    const isEasy = config.difficulty === 0;
    const [min, max] = strategyConfig.bearDice?.rolls || (isEasy ? [1, 5] : [1, 2]);
    config.survive = randomIntInclusive(min, max);
  }

  return config;
}

export function getBearDiceConfig(opts, positionalConfig, gameEntry, strategyConfig, randomIntInclusive, options = {}) {
...
export function formatBearDiceSettledDetails(rawGameInfo = {}) {
  ...
  return {
    difficulty,
    difficulty_name: DIFFICULTY_NAMES[difficulty] || `Risk ${difficulty}`,
    num_runs: numRuns,
