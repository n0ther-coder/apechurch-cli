...
import { ensureIntRange, randomIntInclusive as defaultRandomIntInclusive } from '../utils.js';
...
function buildGimbozSmashConfigFromIntervals(intervals = [], metadata = {}) {
  ...
  if (metadata.outRange || metadata.cover !== undefined) {
    return {
      ...config,
      ...(metadata.outRange ? { outRange: metadata.outRange } : {}),
...
function parseHumanTargetToken(token, {
  ...
  invalidPrefix = 'Invalid range',
  allowSingleNumber = kind === 'out-range',
} = {}) {
  ...
  if (!allowSingleNumber && match[2] === undefined) {
    throw new Error(`${invalidPrefix}: "${trimmed}". A single number is ambiguous for Gimboz Smash. Use --cover ${match[1]} to randomize a ${match[1]}-number cover, or --range ${match[1]}-${match[1]} to target exactly ${match[1]}.`);
  }
...
export function parseGimbozSmashInput(
  { range, outRange, cover, defaultRange } = {},
  { allowEmpty = false, randomIntInclusive = defaultRandomIntInclusive } = {},
) {
  const insideSelection = resolveGimbozSmashInsideSelection({ range });
  const hasInsideSelection = insideSelection !== '';
  const hasOutRange = hasConfiguredValue(outRange);
  const hasCover = hasConfiguredValue(cover);

  if ([hasInsideSelection, hasOutRange, hasCover].filter(Boolean).length > 1) {
    throw new Error('Invalid Gimboz Smash config: choose only one of --range, --out-range, or --cover.');
  }

  if (hasCover) {
    return buildRandomGimbozSmashConfigFromCover(cover, randomIntInclusive);
  }
...
function insideSelectionFromCoverIndex(cover, index) {
  const start = HUMAN_MIN_TARGET + index;
  const end = start + cover - 1;
  return { start, end };
}

function outsideSelectionFromCoverIndex(cover, index) {
  const excludedCount = BOARD_SIZE - cover;
  const start = HUMAN_MIN_TARGET + index;
  const end = start + excludedCount - 1;
  return { start, end };
}

export function buildRandomGimbozSmashConfigFromCover(cover, randomIntInclusive = defaultRandomIntInclusive) {
  const normalizedCover = ensureIntRange(cover, 'cover', MIN_WIN_COUNT, MAX_WIN_COUNT);
  const insideCount = BOARD_SIZE - normalizedCover + 1;
  const outsideCount = normalizedCover + 1;
  const index = randomIntInclusive(0, insideCount + outsideCount - 1);

  if (index < insideCount) {
    return buildGimbozSmashConfigFromIntervals(
      [insideSelectionFromCoverIndex(normalizedCover, index)],
      { cover: normalizedCover },
    );
  }

  const outRange = outsideSelectionFromCoverIndex(normalizedCover, index - insideCount);
  return buildGimbozSmashConfigFromIntervals(
    buildComplementIntervals(outRange),
    { outRange: formatHumanRange(outRange), cover: normalizedCover },
  );
}
  ...
  const explicitConfig = parseGimbozSmashInput(
    {
      ...
      cover: opts.cover ?? positionalConfig.cover,
    },
    { allowEmpty: true, randomIntInclusive },
  );
  if (explicitConfig) {
