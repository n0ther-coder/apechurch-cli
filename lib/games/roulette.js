 ...
 *
 * Handles the "1-wei bug": Single bets need to subtract 1 wei
 * to avoid a contract edge case. This is a known quirk of the
 * roulette contract implementation.
 *
...
export function calculateRouletteBetAmounts(totalWagerWei, gameNumbers) {
  ...
  // This prevents a contract edge case with exact amounts
 ...
 * 3. Calculate bet amounts (split evenly, handle 1-wei bug)
...
export async function playRoulette({
  ...
  // Validate referral address (returns default team address if not set)
  const refAddress = getValidRefAddress(referral);

  // Generate unique identifiers
  const gameId = randomUint256();           // Unique game ID
  const userRandomWord = randomBytes32();   // Client-side entropy for VRF
