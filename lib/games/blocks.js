 ...
 *
 * Blocks is a batched 3x3 board game:
 * - Each run fills a 3x3 board with random colors
 * - The largest connected color cluster determines the payout
 * - Mode 0 ("Easy"/"Low") starts paying at 3 connected tiles
 * - Mode 1 ("Hard"/"High") removes that low-floor payout and raises the cap
 *
 * On-chain encoding:
 * - riskMode: uint8 (0-1)
 * - numRuns: uint8 (1-5)
...
export async function playBlocks({
  ...
  const config = {
    mode: modeValue,
    modeName: MODE_NAMES[modeValue] || 'Easy',
    runs: runsValue,
  };
...
export function getBlocksConfig(opts, positionalConfig, gameEntry, strategyConfig, randomIntInclusive) {
  const config = {};
