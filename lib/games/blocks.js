...
export async function playBlocks({
  ...
  const requestedRuns = resolveBlocksSurviveInput(survive, runs, rolls);
...
export function getBlocksConfig(opts, positionalConfig, gameEntry, strategyConfig, randomIntInclusive, options = {}) {
  ...
  const requestedRuns = resolveBlocksSurviveInput(opts.survive, opts.runs, opts.rolls);
  if (requestedRuns !== undefined) {
    config.survive = parseInt(requestedRuns, 10);
