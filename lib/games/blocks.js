 ...
 * - Mode 0 ("Low") starts paying at 3 matching tiles
...
export async function playBlocks({
  ...
  const requestedRuns = resolveBlocksRunsInput(runs, rolls);
...
function resolveBlocksRunsInput(runs, rolls) {
  ...
  if (hasRuns && hasRolls) {
    ...
    if (Number.isFinite(parsedRuns) && Number.isFinite(parsedRolls) && parsedRuns !== parsedRolls) {
      throw new Error('Conflicting Blocks roll count: --runs and --rolls must match, or only one may be provided.');
    }
  }
...
export function getBlocksConfig(opts, positionalConfig, gameEntry, strategyConfig, randomIntInclusive, options = {}) {
  ...
  const requestedRuns = resolveBlocksRunsInput(opts.runs, opts.rolls);
  if (requestedRuns !== undefined) {
    config.runs = parseInt(requestedRuns, 10);
