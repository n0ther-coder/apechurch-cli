 ...
 * @param {number} [params.split] - Number of independent ball drops, validated against the selected game config
 * @param {number} [params.balls] - Legacy internal alias for split count
...
export async function playPlinko({
  ...
  const ballsValue = ensureIntRange(
    split ?? balls ?? gameEntry.config.balls.default,
    'split',
 ...
 * 1. Explicit CLI flag (--risk, --split)
...
export function getPlinkoConfig(opts, positionalConfig, gameEntry, strategyConfig, randomIntInclusive, options = {}) {
  ...
  // Resolve independent split count.
  if (opts.split !== undefined) {
    config.split = parseInt(opts.split);
  } else if (positionalConfig.balls !== undefined) {
    config.split = positionalConfig.balls;
  } else if (preferGameDefault) {
    config.split = gameEntry.config.balls.default;
  } else {
    ...
    config.split = randomIntInclusive(ballMin, ballMax);
  }
