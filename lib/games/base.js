...
import { EXECUTOR_FEE_ABI, GAME_CONTRACT_ABI, PLINKO_VRF_ABI, SLOTS_VRF_ABI } from '../constants.js';
import { sanitizeError, randomBytes32, randomUint256, getValidRefAddress, formatApeAmount } from '../utils.js';
import { estimateGpFromWagerApe, resolveGpPerApe, saveGameToHistory } from '../profile.js';
import { resolveConfiguredGameVariant } from '../rtp.js';
import { logTransactionRetry, retryBroadcast } from '../tx-resilience.js';
import { getGameDisplayName } from '../../registry.js';

const UINT256_MAX = (1n << 256n) - 1n;
const GAS_BUFFER_BPS = 11500n;
const BPS_DENOMINATOR = 10000n;

function isGenericTransactionFailure(message) {
  return !message
    || message === 'Unknown error'
    || /unknown error occurred while executing the contract function/i.test(message);
}

function parseUint256Override(value, label) {
  ...
  try {
    ...
    throw new Error(`${label} must be a uint256 decimal or 0x-prefixed hex value.`);
  }

  if (parsed < 0n || parsed > UINT256_MAX) {
    throw new Error(`${label} must be within uint256 range.`);
  }

  return parsed;
}

function parseAddressOverride(value, label) {
  if (value === undefined || value === null || String(value).trim() === '') {
    return null;
  }
...
function parseBytes32Override(value, label) {
  if (value === undefined || value === null || String(value).trim() === '') {
    return null;
  }
...
export function resolveGamePayloadInputs({ referral, xGameId, xRef, xUserRandomWord } = {}) {
  return {
    gameId: parseUint256Override(xGameId, '--x-gameId') ?? randomUint256(),
    refAddress: parseAddressOverride(xRef, '--x-ref') ?? getValidRefAddress(referral),
    userRandomWord: parseBytes32Override(xUserRandomWord, '--x-userRandomWord') ?? randomBytes32(),
  };
}
...
export async function getExecutorFee(publicClient, contractAddress) {
  try {
    return await publicClient.readContract({
      address: contractAddress,
      abi: EXECUTOR_FEE_ABI,
      functionName: 'EXECUTOR_FEE',
    });
  } catch (error) {
    throw new Error(`Failed to read executor fee: ${sanitizeError(error)}`);
  }
}
 ...
 * 1. Transaction submission with automatic retry and optional resilient backoff
 ...
 * 2. Retry once in legacy mode, or use the finite resilient policy queues
 ...
 * @param {Function} [params.fetchSettledResult] - Optional per-game hook to enrich settled results
...
export async function executeGame({
  ...
  try {
    ...
    gas = (gasEstimate * GAS_BUFFER_BPS) / BPS_DENOMINATOR;
  } catch (error) {
    gasEstimateError = error;
    // Fall back to the wallet/provider gas estimation path if RPC estimation fails.
  }
  ...
  // -------------------------------------------------------------------------
  // TRANSACTION SUBMISSION (legacy retry, with optional resilient backoff)
  // -------------------------------------------------------------------------
  let lastError;

  let txHash;
  try {
    txHash = await retryBroadcast({
      resilient,
      legacyDelayMs: 2_000,
  ...
  if (!txHash) {
    const writeError = sanitizeError(lastError);
    const estimateError = gasEstimateError ? sanitizeError(gasEstimateError) : null;
    const message = isGenericTransactionFailure(writeError) && estimateError
      ? estimateError
      : writeError;
  ...
  const effectiveGpPerApe = gpPerApe ?? resolveGpPerApe({ walletAddress: account.address });
  const estimatedGpRaw = estimateGpFromWagerApe({
    wagerApe: parseFloat(formatEther(wager)),
    gpPerApe: effectiveGpPerApe,
  });
  ...
  saveGameToHistory({
    ...
    contract_fee_wei: vrfFee.toString(),
    gp_received_raw: estimatedGpRaw,
  ...
  for (let pollAttempt = 0; pollAttempt < maxPollAttempts; pollAttempt++) {
    ...
    try {
      ...
      if (hasEndeds[0]) {
        ...
        if (typeof fetchSettledResult === 'function') {
          try {
            const extraSettledResult = await fetchSettledResult({
              publicClient,
              contractAddress,
              gameId,
              gameEntry,
            ...
            if (extraSettledResult && typeof extraSettledResult === 'object') {
              Object.assign(settledResult, extraSettledResult);
            }
          } catch {
            // Non-essential detail fetch failed. Keep the core settlement response.
          }
        }

        response.result = settledResult;
        saveGameToHistory({
          contract: contractAddress,
          gameId: gameId.toString(),
          timestamp: Date.now(),
          ...
          config,
          settled: true,
          wager_wei: buyIns[0].toString(),
          payout_wei: payouts[0].toString(),
          contract_fee_wei: vrfFee.toString(),
          gas_fee_wei: '0',
          last_sync_on: new Date().toISOString(),
          last_sync_msg: 'ok',
          walletAddress: account.address,
        });
        break;
