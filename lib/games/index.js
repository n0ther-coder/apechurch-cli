...
import { playGimbozSmash, getGimbozSmashConfig } from './gimbozsmash.js';
import { playGlydeOrCrash, getGlydeOrCrashConfig } from './glydeorcrash.js';
...
const gameHandlers = {
  ...
  blocks: playBlocks,       // Blocks (selectable max-of-a-kind grid, independent or compounding rolls)
  gimbozsmash: playGimbozSmash, // Gimboz Smash (1-100 target intervals, up to 2 windows)
  speedcrash: playGlydeOrCrash, // Glyde or Crash (fixed-target crash multiplier)
};
...
export const configGetters = {
  ...
  gimbozsmash: getGimbozSmashConfig,
  speedcrash: getGlydeOrCrashConfig,
};
 ...
 * @param {string} params.game - Canonical game key (e.g., 'jungle-plinko', 'roulette')
 ...
 * @param {string} [params.grid] - Blocks grid dimensions: 2x2, 3x3, or 4x4
 * @param {number} [params.gridMode] - Normalized internal Blocks grid mode
 * @param {number} [params.mode] - Internal mode value (legacy surface)
 * @param {number} [params.split] - Number of independent split attempts for Blocks, Plinko, Speed Keno, Primes, or slots
 * @param {number} [params.survive] - Number of all-or-nothing survival attempts for Bear Dice or Blocks
 * @param {number} [params.balls] - Legacy internal alias for Plinko split count
 * @param {number} [params.spins] - Slots-only split alias
 * @param {string} [params.bet] - Bet type (roulette, baccarat)
 * @param {number} [params.range] - Internal win probability % (apestrong)
 * @param {number|string} [params.cover] - Public cover value for ApeStrong or Gimboz Smash
 ...
 * @param {number} [params.rolls] - Legacy internal alias for Bear Dice survival count
 * @param {number} [params.runs] - Legacy internal alias for Bear Dice/Blocks survival count or Primes split count
 * @param {string} [params.targets] - One or two Gimboz Smash target intervals
 * @param {string} [params.outRange] - Gimboz Smash outside range to exclude from the winning set
 * @param {string} [params.multiplier] - Glyde or Crash target multiplier
 * @param {number} [params.timeoutMs] - How long to wait for result (0 = don't wait)
 * @param {string} [params.referral] - Referral address
 * @param {string|number|bigint} [params.xGameId] - Expert override for gameData gameId
 * @param {string} [params.xRef] - Expert override for gameData referral address
 * @param {string} [params.xUserRandomWord] - Expert override for gameData userRandomWord
 *
...
export { getGimbozSmashConfig } from './gimbozsmash.js';
export { getGlydeOrCrashConfig } from './glydeorcrash.js';
