 ...
 * @param {number} [params.split] - Number of independent batched games 1-20 (default: 20)
 * @param {number} [params.games] - Legacy internal alias for split count
...
export async function playSpeedKeno({
  ...
  let numGames = split ?? games ?? gameEntry.config.games.default;
  ...
  const config = {
    ...
    numbers: gameNumbers,
    wagerPerSplit: (Number(wager) / numGames / 1e18).toFixed(4),
  };
