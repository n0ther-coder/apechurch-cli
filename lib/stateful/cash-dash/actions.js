...
export async function startGame({
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
    try {
      ...
      addActiveGame('cash-dash', gameId);
      saveGameToHistory({
        contract: CASH_DASH_CONTRACT,
      ...
      if (attempt < maxRetries && !json) {
        console.log(`   Transaction failed, retrying in 3s... (${sanitizeError(error)})`);
      }
  ...
  throw new Error(`Failed to start ${CASH_DASH_DISPLAY_NAME} after retry: ${sanitizeError(lastError)}`);
}
