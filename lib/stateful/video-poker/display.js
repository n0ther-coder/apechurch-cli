...
function renderGameFull(state, opts = {}) {
  ...
  if (state.awaitingRNG) {
    ...
    lines.push(boxLine(`→ ${getFinalHandSummary(state)}`));
  }
...
export function renderGameFullDecisionEndAuto(state) {
  const cards = getVisibleCards(state);
  return [
    ...buildCardTableBoxLines(cards),
    boxLine(`→ ${getFinalHandSummary(state)}`),
    createBottomBorder(),
  ].join('\n');
}
...
function getFinalHandSummary(state) {
  if (!state?.isComplete || state.handStatus === HandStatus.NOTHING) {
    return 'No winning hand';
  }

  return HandStatusNames[state.handStatus];
}
