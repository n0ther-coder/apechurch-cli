...
const VIDEO_POKER_DISPLAY_NAME_UPPER = VIDEO_POKER_DISPLAY_NAME.toUpperCase();

function getNetProfitApe(state) {
  return (Number(state?.totalPayoutApe) || 0) - (Number(state?.betAmountApe) || 0);
}
...
function renderGameSimple(state, opts = {}) {
  ...
  if (showHeader) {
    ...
    const header = `  🃏 ${VIDEO_POKER_DISPLAY_NAME_UPPER}  │  Bet: ${state.betAmountApe} APE${gameLabel}`;
  ...
  if (state.awaitingRNG) {
    ...
    const outcomeIcon = formatOutcomeIcon(netProfitApe);
    if (state.handStatus === HandStatus.NOTHING) {
      lines.push(`  ${outcomeIcon} No winning hand ${formatNetProfitLabel(netProfitApe)}`);
    } else {
      const payout = state.totalPayoutApe;
      lines.push(`  ${outcomeIcon} ${HandStatusNames[state.handStatus]}! → ${payout} APE (${PAYOUTS[state.handStatus]}x) ${formatNetProfitLabel(netProfitApe)}`);
    }
  }
...
function renderGameFull(state, opts = {}) {
  const lines = [
    createTopBorder(VIDEO_POKER_DISPLAY_NAME_UPPER),
    boxLine(''),
  ];
...
export function formatOutcomeFooter(state) {
  ...
  if (!state?.isComplete || state.handStatus === HandStatus.NOTHING) {
    return `${outcomeIcon} No winning hand ${formatNetProfitLabel(netProfitApe)}`;
  }

  return `${outcomeIcon} ${HandStatusNames[state.handStatus]}! → ${state.totalPayoutApe} APE (${PAYOUTS[state.handStatus]}x) ${formatNetProfitLabel(netProfitApe)}`;
}
...
│ Jacks or Better    │    1x  │
└────────────────────┴────────┘
  * Max bet (${BET_AMOUNTS[MAX_BET_INDEX]} APE) = Jackpot eligible on Royal Flush
`;
}
