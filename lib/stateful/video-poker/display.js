...
const MAX_VIDEO_POKER_HEADER = `  🃏 VIDEO POKER  │  Bet: ${Math.max(...BET_AMOUNTS)} APE  |  Game #10000 /10000`;
const VIDEO_POKER_DIVIDER_WIDTH = Math.ceil(MAX_VIDEO_POKER_HEADER.length * 1.2);
...
export function renderGame(state, opts = {}) {
  ...
  const divider = '─'.repeat(VIDEO_POKER_DIVIDER_WIDTH);
...
function formatDecisionCardsLine(cards, handHint) {
  ...
  return `${cardsLine.padEnd(cardsLine.length + 8)}📊  ${handHint}`;
}
