...
async function gameLoop(account, publicClient, walletClient, initialState, opts = {}) {
  ...
  const output = renderGame(state, { displayMode, gameLabel, showHeader: true });
  console.log(output);
  ...
  if (state.awaitingRNG) {
    ...
    const output2 = renderGame(state, { displayMode, gameLabel, showHeader: false });
    console.log(output2);
    maybePlayWinChime(state, isJson);
  }
  ...
  if (state.awaitingDecision) {
    ...
    // Show final result
    const finalOutput = renderGame(state, { displayMode, gameLabel, showHeader: false });
