...
async function gameLoop(account, publicClient, walletClient, initialState, opts = {}) {
  ...
  // If awaiting decision, prompt for discards
  if (state.awaitingDecision) {
    ...
    if (autoPlay) {
      ...
      console.log('  🤖 Auto-play: Keeping all cards (strategy TBD)\n');
    } else {
      // Interactive: prompt for discards
      const input = await prompt(renderDiscardPrompt());
      cardsToRedraw = parseDiscardInput(input);
    }
    
    const discardCount = cardsToRedraw.filter(Boolean).length;
    
    if (!isJson) {
      // Show confirmation of what's being kept/discarded
      console.log('');
      console.log(formatDiscardConfirmation(state.initialCards, cardsToRedraw));
      console.log('');
      console.log('  Sending transaction...');
    }
