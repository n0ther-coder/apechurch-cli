...
export async function start(amount, opts = {}) {
  ...
  const loopDelaySeconds = parseLoopDelaySeconds(opts.delay, DEFAULT_LOOP_DELAY_SECONDS);
  ...
  if (!Number.isFinite(loopDelaySeconds) || loopDelaySeconds < 1) {
  ...
  while (true) {
    ...
    console.log(`\n⏳ Next game in ${formatDelayMs(nextDelayMs)} | 💰 Balance: ${currentApe.toFixed(2)} APE (${changeStr})\n`);
    ...
    if (autoPlay) {
      if (bestAuto) {
        try {
          ...
          const bestDecision = getBestHoldByEV(state.initialCards, {
        ...
        // Auto-play: use simple priority strategy
        const optimal = getOptimalHold(state.initialCards);
        cardsToRedraw = holdToRedraw(optimal.hold);
