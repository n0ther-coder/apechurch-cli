 ...
 * - Fixed denomination bets (10, 25, 50, 100, 250, 400 APE)
...
const VIDEO_POKER_DISPLAY_NAME = resolveGameDisplayName({
  gameKey: 'video-poker',
...
async function checkUnfinished(opts = {}) {
  ...
  console.log(`\n⚠️  You have ${count} unfinished ${VIDEO_POKER_DISPLAY_NAME} game${count > 1 ? 's' : ''}.`);
...
export async function start(amount, opts = {}) {
  ...
  function findClosestBet(targetBet, availableBalance) {
    ...
    const floor = minBet || 0;
    const cap = maxBet || Infinity;
    const affordableBets = BET_AMOUNTS.filter(b => b <= availableBalance && b >= floor && b <= cap);
    if (affordableBets.length === 0) return null;

    const atOrBelowTarget = affordableBets.filter(b => b <= targetBet);
    if (atOrBelowTarget.length > 0) {
      return atOrBelowTarget[atOrBelowTarget.length - 1];
    }
    return affordableBets[0];
  }

  let pendingResumeGameId = null;
  const autoResumeEnabled = isAutoModeEnabled(autoMode);
  // Check for unfinished games
  let existingGameId = await checkUnfinished({
    json: isJson,
    autoResume: autoResumeEnabled,
  });
  while (existingGameId) {
    if (!loopMode) {
      const resumed = await resume(existingGameId, opts);
      if (autoResumeEnabled && !isJson && resumed?.status === 'missing') {
        const nextGameId = getOldestActiveGame('video-poker');
        if (nextGameId) {
          console.log(`   Cleared stale unfinished ${VIDEO_POKER_DISPLAY_NAME} game ${existingGameId}; checking next queued game.\n`);
          existingGameId = nextGameId;
          continue;
        }
        console.log(`   Cleared stale unfinished ${VIDEO_POKER_DISPLAY_NAME} game ${existingGameId}; starting a new game.\n`);
        break;
      }
      return resumed;
    }
  ...
  const betValidation = (amount === undefined || amount === null) && betStrategy.requiresNoBaseBet
    ? { valid: true, index: 0, amount: 0 }
  ...
  while (true) {
    ...
    if (pendingResumeGameId) {
      ...
      if (!resumed || resumed.status !== 'completed') {
        if (!isJson) {
          console.log(`\n🛑 Stopping: unfinished ${VIDEO_POKER_DISPLAY_NAME} game still needs manual intervention.\n`);
        }
      ...
      if (loopMode && !isJson && !loopEstimateShown) {
        if (currentBetIndex === MAX_BET_INDEX) {
          try {
            jackpotApe = parseFloat(formatEther(await getJackpotTotal(publicClient)));
          } catch {
            jackpotApe = null;
          }
        }
      ...
      if (verbose && !isJson && currentBetIndex === MAX_BET_INDEX) {
        if (jackpotApe === null) {
          const jackpot = await getJackpotTotal(publicClient);
      ...
      if (verbose && !isJson) {
        console.log(`\n🃏 Starting ${VIDEO_POKER_DISPLAY_NAME} - ${currentBet} APE bet${formatLoopGameLabel(gamesPlayed + 1, maxGames) ? ` [${formatLoopGameLabel(gamesPlayed + 1, maxGames)}]` : ''}`);
...
function printResumeQueue(gameIds) {
  console.log(`\n🧩 Unfinished ${VIDEO_POKER_DISPLAY_NAME} Games (${gameIds.length})\n`);
...
async function resumeSingleGame(gameId, opts = {}) {
  ...
  if (verbose && !isJson) {
    console.log(`\n🃏 Resuming ${VIDEO_POKER_DISPLAY_NAME} - Game ${gameId}`);
...
export async function resume(gameIdInput, opts = {}) {
  ...
  if (gameIds.length === 0) {
    if (isJson) console.log(JSON.stringify({ error: `No active ${VIDEO_POKER_DISPLAY_NAME} games` }));
    else console.error(`\n❌ No active ${VIDEO_POKER_DISPLAY_NAME} games to resume\n`);
  ...
  for (const [index, gameId] of gameIds.entries()) {
    if (!gameIdInput && !isJson) {
      console.log(`▶️  Resuming ${VIDEO_POKER_DISPLAY_NAME} game ${index + 1}/${gameIds.length}: ${gameId}\n`);
    }
    ...
    if (!gameIdInput && !isJson && index < gameIds.length - 1) {
      console.log(`🛑 Stopping batch resume. Remaining unfinished ${VIDEO_POKER_DISPLAY_NAME} games stay queued.\n`);
    }
    break;
  }
...
export async function status(gameIdInput, opts = {}) {
  ...
  if (!gameId) {
    ...
    if (!gameId) {
      ...
      console.log(`\nNo active ${VIDEO_POKER_DISPLAY_NAME} games.\n`);
      return;
    }
  }
...
async function gameLoop(account, publicClient, walletClient, initialState, opts = {}) {
  ...
  if (state.awaitingDecision) {
    ...
    try {
      const redrawResult = await executeRedraw({
        account,
        publicClient,
        walletClient,
        gameId,
      ...
      lastActionTxHash = redrawResult?.hash ?? null;
      if (discardCount > 0) {
