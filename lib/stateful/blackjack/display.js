...
import { AUTO_MODE_MAX } from '../auto.js';
...
export function renderGame(state, actions, opts = {}) {
  ...
  switch (mode) {
    case 'json':
      return renderGameJson(state, actions, opts);
    case 'simple':
...
function renderGameFull(state, actions, opts = {}) {
  ...
  if (state.isComplete) {
    ...
    const netApe = formatApe(result.net);
    const outcomeIcon = formatOutcomeIcon(netApe);
    lines.push('  ─────────────────────────────────────');
    if (result.won) {
      lines.push(`  ${outcomeIcon} YOU WIN! ${formatNetProfitLabel(netApe)}`);
    } else if (result.push) {
      lines.push(`  ${outcomeIcon} PUSH ${formatNetProfitLabel(netApe)}`);
    } else {
      lines.push(`  ${outcomeIcon} DEALER WINS ${formatNetProfitLabel(netApe)}`);
    }
...
function renderGameSimple(state, actions, opts = {}) {
  ...
  if (state.isComplete) {
    ...
    lines.push('');
    if (result.won) {
      lines.push(`${outcomeIcon} RESULT: WIN ${formatNetProfitLabel(netApe)}`);
    } else if (result.push) {
      lines.push(`${outcomeIcon} RESULT: PUSH ${formatNetProfitLabel(netApe)}`);
    } else {
      lines.push(`${outcomeIcon} RESULT: LOSE ${formatNetProfitLabel(netApe)}`);
    }
  }
 ...
 */
function renderGameJson(state, actions, opts = {}) {
  const result = state.isComplete ? calculateNetResult(state) : null;
  const autoDecisions = Array.isArray(opts.autoDecisions) ? opts.autoDecisions : [];
  const solverBudgetMode = opts.autoMode || opts.solverMode || null;
  ...
  if (opts.autoMode || autoDecisions.length > 0) {
    output.autoMode = opts.autoMode || null;
    output.effectiveSolver = getEffectiveSolver(autoDecisions);
    output.solverDecisionCount = autoDecisions.length;
    output.solverFallbacks = getSolverFallbacks(autoDecisions);
  }

  if (opts.solverMode || opts.solverSuggestion) {
    output.solverMode = opts.solverMode || null;
    output.solverSuggestion = opts.solverSuggestion || null;
  }
  
  return JSON.stringify(output, null, 2);
}

function getEffectiveSolver(autoDecisions) {
  const solvers = [...new Set(
    autoDecisions
      .map((decision) => decision?.effectiveSolver)
      .filter(Boolean)
  )];

  if (solvers.length === 0) return null;
  if (solvers.length === 1) return solvers[0];
  return 'mixed';
}

function getSolverFallbacks(autoDecisions) {
  return autoDecisions
    .filter((decision) => decision?.fallbackError)
    .map((decision) => ({
      sequence: decision.sequence,
      requestedSolver: decision.requestedSolver || null,
      effectiveSolver: decision.effectiveSolver || null,
      fallbackError: decision.fallbackError,
      solverTimedOut: decision.solverTimedOut || false,
      solverElapsedMs: decision.solverElapsedMs ?? null,
      solverMaxStates: decision.solverMaxStates ?? null,
      solverTimeoutMs: decision.solverTimeoutMs ?? null,
    }));
}
