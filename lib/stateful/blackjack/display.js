...
/**
 * Full ASCII display with card art
 */
function renderGameFull(state, actions, opts = {}) {
  ...
  if (dealerCards.length > 0) {
    const cardLines = renderCardsCompact(dealerCards, dealerFaceDown);
    lines.push(...cardLines.map(l => '  ' + l));
    
    // Show dealer value if no face-down card
    if (dealerFaceDown.length === 0 && dealerCards.length > 0) {
      const valueStr = formatHandValue(state.dealerHand);
      lines.push(`  ${valueStr}`);
    } else if (dealerCards.length === 1) {
      lines.push(`  Showing: ${state.dealerHand.cards[0].value}`);
    }
  }
  ...
  for (let i = 0; i < numHands; i++) {
    ...
    lines.push(label);
    const cardLines = renderCardsCompact(hand.cards, []);
    lines.push(...cardLines.map(l => '  ' + l));
    
    // Hand info
    const valueStr = formatHandValue(hand);
    const betStr = `Bet: ${formatApe(hand.bet)} APE`;
    lines.push(`  ${valueStr}  |  ${betStr}`);
    lines.push('');
  }
  
  // Insurance/Side bets info
  if (state.insuranceBet.hasBet) {
    lines.push(`  Insurance: ${formatApe(state.insuranceBet.bet)} APE`);
    lines.push('');
  }
  ...
  // Build box
  const boxed = boxContent(lines, 'BLACKJACK', width);
  return boxed.join('\n');
}
...
function renderGameSimple(state, actions, opts = {}) {
  ...
  for (let i = 0; i < numHands; i++) {
    ...
    const valueStr = formatHandValue(hand);
    const betStr = `(${formatApe(hand.bet)} APE)`;
...
/**
 * Render cards in compact 3-line format
 */
function renderCardsCompact(cards, faceDownIndices = []) {
  ...
  const rendered = cards.map((card, i) => {
    ...
    // Use card.rank and card.suit if available, else compute from rawCard
    ...
    const r = rank.length === 2 ? rank : rank + ' ';
    return ['┌────┐', `│${r}${suit}│`, '└────┘'];
  });
