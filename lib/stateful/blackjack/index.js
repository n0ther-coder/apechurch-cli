...
export async function start(amount, opts = {}) {
  ...
  while (true) {
    ...
    // Check target profit (loop mode)
    if (loopMode && targetProfit !== null) {
      const currentProfit = balanceApe - startingBalance;
      if (currentProfit >= targetProfit) {
        console.log(`\n🎯 Target reached! Profit: ${currentProfit.toFixed(2)} APE (target: ${targetProfit} APE)`);
        printSessionStats(gamesPlayed, startingBalance, balanceApe);
        break;
      }
    }
    ...
    if (!isJson) {
      ...
      console.log(`\n🎰 Starting Blackjack - ${betApe} APE bet${gameNum}`);
    ...
    // Small delay between games in loop mode
