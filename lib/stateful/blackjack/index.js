...
export async function start(amount, opts = {}) {
  ...
  while (true) {
    ...
    // Start the game
    if (verbose && !isJson) {
      console.log(`\n🎰 Starting Blackjack - ${formatBlackjackStake(currentBet)} APE bet${formatLoopGameLabel(gamesPlayed + 1, maxGames) ? ` [${formatLoopGameLabel(gamesPlayed + 1, maxGames)}]` : ''}`);
    ...
    const terminalConditionReached = (
      ...
      (targetProfit !== null && balanceAfterGame >= targetProfit) ||
