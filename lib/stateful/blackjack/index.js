...
function parseSolverMaxStates(rawValue) {
  if (rawValue === undefined || rawValue === null || rawValue === '') {
    return DEFAULT_AUTO_BEST_MAX_PLAYER_STATES;
  }
...
export async function start(amount, opts = {}) {
  ...
  try {
    solverMaxStates = parseSolverMaxStates(opts.solverMaxStates);
  } catch (error) {
    const err = { error: error.message };
    if (isJson) return console.log(JSON.stringify(err));
    console.error(`\n❌ ${err.error}\n`);
...
async function gameLoop(account, publicClient, walletClient, initialState, opts = {}) {
  ...
  while (true) {
    ...
    if (autoPlay) {
      ...
      selectedAction = getAutoPlayAction(state, actions, { autoMode, solverMaxStates });
 ...
 */
function getAutoPlayAction(state, actions, { autoMode, solverMaxStates } = {}) {
  const affordableActions = actions.filter(a => a.canAfford);
