...
export async function start(amount, opts = {}) {
  ...
  try {
    solverMaxStates = parseSolverMaxStates(opts.solverMaxStates, { autoMode });
    solverTimeoutMs = parseSolverTimeoutMs(opts.solverTimeoutMs, { autoMode });
  } catch (error) {
    const err = { error: error.message };
    if (isJson) return console.log(JSON.stringify(err));
    console.error(`\n❌ ${err.error}\n`);
...
async function gameLoop(account, publicClient, walletClient, initialState, opts = {}) {
  ...
  const solverMaxStates = opts.solverMaxStates ?? getDefaultSolverMaxStates(autoMode);
  const solverTimeoutMs = opts.solverTimeoutMs ?? getDefaultSolverTimeoutMs(autoMode);
