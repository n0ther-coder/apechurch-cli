...
export function getAvailableActions(state, balance, vrfFee) {
  ...
  if (isFirstAction && state.dealerHand.cards[0]?.value === 11) {
    const insuranceCost = initialBet / 2n;
    ...
    actions.push({
      ...
      cost: insuranceCost,
      canAfford,
      shortfall: canAfford ? 0n : insuranceCost - balance,
    });
  }
...
/**
 * Format action for display with cost
 */
export function formatActionLabel(action, showCost = true) {
  if (!showCost || action.cost === 0n) {
    return action.label;
  }
  const costApe = parseFloat(formatEther(action.cost)).toFixed(2);
  return `${action.label} (+${costApe} APE)`;
}
