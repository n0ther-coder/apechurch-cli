 ...
 * Basic strategy for the live Blackjack+ rule surface (dealer hits soft 17).
 */
...
const SOFT_STRATEGY = {
  ...
  6:  [D, D, D, D, D, H, H, H, H, H],  // A,6 = soft 17
...
export function getOptimalAction(playerCards, dealerUpcard, options = {}) {
  ...
  if (action === D && !canDouble) {
    action = getDoubleFallbackAction(hand);
    reason += ` (double unavailable, ${action})`;
  }
  
  if (action === R && !canSurrender) {
    action = getSurrenderFallbackAction(hand);
    reason += ` (surrender unavailable, ${action})`;
  }
  
  return { action, reason };
}

function getDoubleFallbackAction(hand) {
  if (hand.isSoft && hand.total >= 18) {
    return S;
  }
...
function getSurrenderFallbackAction(hand) {
  if (hand.total >= 17) {
    return S;
  }
