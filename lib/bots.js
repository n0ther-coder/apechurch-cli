...
export function resolveBotsDir() {
  ...
  if (!overrideDir) {
    return path.join(APECHURCH_DIR, BOTS_DIR_BASENAME);
  }

  const normalizedOverride = path.resolve(overrideDir);
  if (path.basename(normalizedOverride) === BOTS_DIR_BASENAME) {
    return normalizedOverride;
  }

  return path.join(normalizedOverride, BOTS_DIR_BASENAME);
}
