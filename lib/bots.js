...
function buildBotRunSummaryFields(metadata, { endedAt = new Date().toISOString() } = {}) {
  return {
...
export function createBotRuntimeContext(bot, { cliPath, rawArgs = [], runMetadata = null } = {}) {
  if (!cliPath) {
  ...
  const metadata = runMetadata || createBotRunMetadata(bot, { rawArgs: runtimeArgs });
...
function writeBotSummaryLog(botCommand, summary) {
  ...
  const basePath = path.join(LOG_DIR, `${botCommand}.${timestamp}.json`);
  let filePath = basePath;
  let suffix = 1;

  while (fs.existsSync(filePath)) {
    filePath = path.join(LOG_DIR, `${botCommand}.${timestamp}.${suffix}.json`);
    suffix += 1;
  }
...
function enrichBotRunSummary(summary, bot, metadata, ctx = null) {
  if (!summary || typeof summary !== 'object' || Array.isArray(summary)) {
    return summary;
  }
  ...
  const enriched = {
    ...
    ...buildBotRunSummaryFields(metadata),
  };
...
function createBotRunLogFinalizer(bot, metadata) {
  ...
  function buildFailureSummary(status, { error = null, signal = null, ctx = null } = {}) {
    const summary = {
      bot: bot.command,
      bot_name: bot.name,
      status,
    ...
    return enrichBotRunSummary(summary, bot, metadata, ctx);
  }
...
export async function runBot(bot, runtimeOptions = {}) {
  ...
  try {
    ...
    if (result.summary) {
      const summary = enrichBotRunSummary(result.summary, bot, runMetadata, ctx);
      logFinalizer.writeSummary(summary);
