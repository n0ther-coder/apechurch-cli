...
function getBooleanArgFlag(args = [], optionName, negatedOptionName = null) {
  ...
  for (const token of tokens) {
    ...
    const rawValue = token.slice(optionName.length + 1).trim().toLowerCase();
    if (['1', 'true', 'yes', 'y'].includes(rawValue)) result = true;
    if (['0', 'false', 'no', 'n'].includes(rawValue)) result = false;
  }
  return result;
}
