...
import { createR2LogMirror } from './r2.js';
import {
  loadProfile,
  resolveGpPerApeInfo,
} from './profile.js';
...
const BOT_SIGNAL_R2_FLUSH_TIMEOUT_MS = 1_500;
...
const TRANSACTION_RETRY_LINE_RE = /^\s*⚠️\s+.+, Rechecking in .+ \(at .+\)\.\s*$/;
  ...
  return new Promise((resolve, reject) => {
    ...
    child.stderr.on('data', (chunk) => {
      ...
      if (options.forwardStderr) {
        forwardedStderrRemainder = forwardNestedBotStderrChunk(chunk, forwardedStderrRemainder, {
          ...
          lineFilter: options.forwardStderrFilter,
        });
      }
    ...
    child.on('exit', (code, signal) => {
      ...
      if (options.forwardStderr && forwardedStderrRemainder) {
        if (!options.forwardStderrFilter || options.forwardStderrFilter(forwardedStderrRemainder)) {
...
function forwardNestedBotStderrChunk(chunk, remainder, { colorOutput = false, lineFilter = null } = {}) {
  const text = `${remainder || ''}${chunk}`;
  const lines = text.split('\n');
  const nextRemainder = lines.pop() || '';

  for (const line of lines) {
    if (typeof lineFilter === 'function' && !lineFilter(line)) continue;
...
function isTransactionRetryStderrLine(line) {
  return TRANSACTION_RETRY_LINE_RE.test(stripAnsi(line));
}
...
function getBooleanArgFlag(args = [], optionName, negatedOptionName = null) {
  ...
  for (const token of tokens) {
    ...
    if (negatedOptionName && token === negatedOptionName) {
      result = false;
      continue;
    }
  }
...
function appendInheritedBooleanArg(tokens, optionName, enabled, negatedOptionName = null) {
  const normalized = tokens.map((arg) => String(arg));
  if (!enabled) return normalized;
  if (normalized.some((token) => (
    token === optionName
    || (negatedOptionName && token === negatedOptionName)
  ))) {
    return normalized;
  }
...
function extractGpPerApeArg(rawArgs = []) {
  const tokens = Array.isArray(rawArgs) ? rawArgs.map((arg) => String(arg)) : [];
  for (let index = 0; index < tokens.length; index += 1) {
    const token = tokens[index];
    if (token === '--gp-ape') {
      return parsePositiveNumberOption(tokens[index + 1]);
    }
    if (token.startsWith('--gp-ape=')) {
      return parsePositiveNumberOption(token.slice('--gp-ape='.length));
    }
  }
  return null;
}

function resolveBotRunGpRate(metadata = {}) {
  try {
    const cliGpPerApe = extractGpPerApeArg(metadata.rawArgs);
    const profile = loadProfile();
    const info = resolveGpPerApeInfo({ cliGpPerApe, profile });
    return {
      base_gp_per_ape: info.baseGpPerApe,
      current_gp_per_ape: info.currentGpPerApe,
      effective_gp_per_ape: info.gpPerApe,
      source: info.source,
...
export function createBotRuntimeContext(bot, {
  ...
  const inheritedResilient = getBooleanArgFlag(runtimeArgs, '--resilient', '--no-resilient');
  ...
  function withInheritedPlayArgs(tokens) {
    return appendInheritedBooleanArg(tokens, '--resilient', inheritedResilient, '--no-resilient');
  }
  ...
  const runtimeContext = {
    ...
    session: botSession,
    resolveGame(command) {
      if (typeof gameResolver !== 'function') return null;
      return gameResolver(command) || null;
    },
    resolveBot(command) {
      if (typeof botResolver !== 'function') return null;
      return botResolver(command) || null;
    },
    async play(args, options = {}) {
      const tokens = withInheritedPlayArgs(normalizePlayArgs(args));
    ...
    async playJson(args, options = {}) {
      const tokens = normalizeJsonPlayArgs(withInheritedPlayArgs(normalizePlayArgs(args)));
      const env = createNestedPlayEnv({
        forceChime: shouldForceChildPlayChime(),
        ...options,
      });
      const result = await runChildProcessCapture(
        ...
        {
          forwardStderr: true,
          forwardStderrFilter: isTransactionRetryStderrLine,
        },
      );
      const payload = tryParseJsonPayload(result.stdout);
...
function summaryContainsTransaction(value, seen = new Set()) {
  if (typeof value === 'string') {
    return TRANSACTION_HASH_RE.test(value.trim());
  }
  ...
  if (Array.isArray(value)) {
    return value.some((entry) => summaryContainsTransaction(entry, seen));
  }

  return Object.values(value).some((entry) => summaryContainsTransaction(entry, seen));
}

function isPositiveRecordedWager(value) {
...
function summaryHasPositiveTotalWager(summary) {
  if (!summary || typeof summary !== 'object' || Array.isArray(summary)) return false;
  // Stateful games can settle without returning their transaction hashes to
  // the parent bot. A positive aggregate wager is equivalent gameplay evidence
  // while planned/configured bet fields remain intentionally ignored.
  return TOTAL_WAGER_KEYS.some((key) => isPositiveRecordedWager(summary[key]));
}
...
function enrichBotRunSummary(summary, bot, metadata, ctx = null, options = {}) {
  ...
  const gpRate = summary.gp_rate && typeof summary.gp_rate === 'object' && !Array.isArray(summary.gp_rate)
    ? summary.gp_rate
    : resolveBotRunGpRate(metadata);
  if (gpRate) {
    enriched.gp_rate = gpRate;
  }
...
async function withBestEffortTimeout(promise, timeoutMs) {
  let timeout;
  try {
    await Promise.race([
      promise,
    ...
    // Best-effort remote logging must never affect bot execution.
  } finally {
...
function createBotRunLogFinalizer(bot, metadata) {
  ...
  const r2Mirror = createR2LogMirror({ logDir: LOG_DIR });
  ...
  function writeRemoteSummary(summary) {
    try {
      r2Mirror.write(filePath, summary);
    } catch {
      // Best-effort mirror: local log writes are authoritative.
    }
  }
  ...
  function writeRawSummary(summary, { ctx = runtimeContext, safe = false, terminal = false } = {}) {
    ...
    if (
      !summaryContainsTransaction(writableSummary)
      && !summaryHasPositiveTotalWager(writableSummary)
    ) {
    ...
    writeRemoteSummary(writableSummary);
  ...
  function installSignalHandlers() {
    for (const signal of BOT_TERMINATION_SIGNALS) {
      const handler = () => {
        ...
        withBestEffortTimeout(r2Mirror.flush(), BOT_SIGNAL_R2_FLUSH_TIMEOUT_MS)
          .finally(() => process.exit(getSignalExitCode(signal)));
      };
...
export async function runBot(bot, runtimeOptions = {}) {
  ...
  try {
    ...
    await withBestEffortTimeout(logFinalizer.flushRemote(), BOT_SIGNAL_R2_FLUSH_TIMEOUT_MS);
