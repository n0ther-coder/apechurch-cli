...
function createBotRunLogFinalizer(bot, metadata) {
  ...
  const filePath = allocateBotSummaryLogPath(bot.command, new Date(metadata.startedAt));
  ...
  function buildWritableSummary(summary, { ctx = runtimeContext, terminal = false } = {}) {
    ...
    const enriched = enrichBotRunSummary(rawSummary, bot, metadata, ctx, {
      endedAt: now,
...
function isHelpOnlyBotInvocation(rawArgs = []) {
  const tokens = Array.isArray(rawArgs) ? rawArgs.map((arg) => String(arg)) : [];
  return tokens.length > 0 && tokens.every((token) => token === '--help' || token === '-h');
}
...
export async function runBot(bot, runtimeOptions = {}) {
  ...
  try {
    ...
    } else if (!isHelpOnlyBotInvocation(runtimeOptions.rawArgs)) {
