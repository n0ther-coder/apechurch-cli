...
function getBooleanArgFlag(args = [], optionName) {
  const tokens = Array.isArray(args) ? args.map((arg) => String(arg)) : [];
  for (const token of tokens) {
    if (token === optionName) return true;
    if (!token.startsWith(`${optionName}=`)) continue;
    const rawValue = token.slice(optionName.length + 1).trim().toLowerCase();
    if (['1', 'true', 'yes', 'y'].includes(rawValue)) return true;
    if (['0', 'false', 'no', 'n'].includes(rawValue)) return false;
  }
  return false;
}

function appendInheritedBooleanArg(tokens, optionName, enabled) {
  const normalized = tokens.map((arg) => String(arg));
  if (!enabled) return normalized;
  if (normalized.some((token) => token === optionName || token.startsWith(`${optionName}=`))) {
    return normalized;
  }
...
export function createBotRuntimeContext(bot, {
  ...
  const inheritedResilient = getBooleanArgFlag(runtimeArgs, '--resilient');
  ...
  function withInheritedPlayArgs(tokens) {
    return appendInheritedBooleanArg(tokens, '--resilient', inheritedResilient);
  }
