...
const ACTIVE_GAME_NAME_OVERRIDES = new Map([
  ...
  ['cash-dash', resolveGameDisplayName({ gameKey: 'cash-dash', fallbackName: 'Cash Dash' })],
...
const ACTIVE_GAME_RESUME_COMMANDS = new Map([
  ...
  ['cash-dash', `${BINARY_NAME} cash-dash resume [--game <id>][--auto [best] | --solver]`],
  ['hi-lo-nebula', `${BINARY_NAME} hi-lo-nebula resume [--game <id>][--auto [mode] | --solver [mode]]`],
...
function resolveStatusSummaryIdentity({
  ...
  const effectiveConfig = (rtpConfig && typeof rtpConfig === 'object') || (config && typeof config === 'object')
    ? {
      ...(config && typeof config === 'object' ? config : {}),
      ...(rtpConfig && typeof rtpConfig === 'object' ? rtpConfig : {}),
    }
    : (rtpConfig || config || null);
  ...
  return {
    ...
    rtpConfig: useResolvedCanonicalVariant
      ...
      : (rtpConfig || resolvedVariant.rtpConfig || effectiveConfig),
  };
}
