...
function getUtcIsoWeekParts(timestampMs) {
  ...
  const weekStartMs = utcMidnight.getTime() - (daysSinceMonday * MS_PER_DAY);

  const thursday = new Date(weekStartMs + (3 * MS_PER_DAY));
...
export function buildHistoryWapeLeaderboard(history) {
  ...
  for (const game of games) {
    ...
    const { year, week, weekStartMs } = getUtcIsoWeekParts(timestampMs);
