...
import { resolveGameByContract, resolveGameDisplayName } from '../registry.js';
import {
  VIDEO_POKER_ABI,
...
export function resolveHistoryGameName(contract) {
  ...
  const gameEntry = resolveGameByContract(normalized);
  if (gameEntry) {
    return resolveGameDisplayName({ contract: normalized, fallbackName: gameEntry.name });
  }
