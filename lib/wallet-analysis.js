...
const LEGACY_BLOCKS_GAME_INFO_ABI = [
  {
    type: 'function',
    name: 'getGameInfo',
    stateMutability: 'view',
...
const currentBlocksHistoryGame = SUPPORTED_GAMES.find((game) => game.key === 'blocks');
const LEGACY_SUPPORTED_GAMES = currentBlocksHistoryGame
  ? [{
      ...currentBlocksHistoryGame,
...
function inferStatelessHistoryVariant(gameEntry, tx) {
  ...
  try {
    switch (gameEntry.key) {
      ...
      case 'blocks': {
        const legacyContract = Boolean(gameEntry.legacyContract)
          || String(gameEntry.contract || '').toLowerCase() === LEGACY_BLOCKS_CONTRACT.toLowerCase();
        ...
        const [gridMode, mode, runs, compounding] = legacyContract
          ? [0, decoded[0], decoded[1], true]
          : decoded;
        const numericGridMode = Number(gridMode);
        const numericMode = Number(mode);
        const isCompounding = Boolean(compounding);
        config = {
          grid: getGameOptionLabel(gameEntry, 'grid', numericGridMode, '3x3'),
          gridMode: numericGridMode,
          mode: numericMode,
          modeName: getGameOptionLabel(gameEntry, 'mode', numericMode, `Mode ${numericMode}`),
          ...(isCompounding ? { survive: Number(runs) } : { split: Number(runs) }),
...
function resolveSupportedStatelessHistoryGame(game) {
  ...
  const supportedGame = SUPPORTED_GAMES_BY_CONTRACT.get(normalizedContract)
    || SUPPORTED_GAMES_BY_KEY.get(normalizedGameKey)
    || null;
  ...
  switch (supportedGame.key) {
    ...
    case 'blocks':
      return {
        ...
        abi: supportedGame.legacyContract ? LEGACY_BLOCKS_GAME_INFO_ABI : BLOCKS_GAME_INFO_ABI,
        functionName: 'getGameInfo',
        args: [BigInt(gameIdString)],
      };
    case 'bear-dice':
...
function inferStatelessHistoryVariantFromGameInfo(gameEntry, rawGameInfo) {
  ...
  switch (gameEntry.key) {
    ...
    case 'blocks': {
      const legacyContract = Boolean(gameEntry.legacyContract);
      const gridMode = legacyContract ? 0 : Number(rawGameInfo.gameMode);
      const mode = Number(rawGameInfo.riskMode);
      const runs = Number(rawGameInfo.numRuns);
      const compounding = legacyContract ? true : Boolean(rawGameInfo.compounding);
      if (!Number.isFinite(gridMode) || !Number.isFinite(mode) || !Number.isFinite(runs)) {
        return null;
      }
      config = {
        grid: getGameOptionLabel(gameEntry, 'grid', gridMode, '3x3'),
        gridMode,
        mode,
        modeName: getGameOptionLabel(gameEntry, 'mode', mode, `Mode ${mode}`),
        ...(compounding ? { survive: runs } : { split: runs }),
        compounding,
      };
      break;
    }
