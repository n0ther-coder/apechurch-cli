...
const HISTORY_DOWNLOAD_CHECKPOINT_CHUNKS = 25;
...
export async function syncSavedStatefulHistoryGames(publicClient, existingGames, walletAddress, syncTimestamp) {
  ...
  const statefulGames = (Array.isArray(existingGames) ? existingGames : []).filter((game) => {
    return isStatefulHistoryContract(game?.contract);
  });
...
export async function downloadWalletHistory(
  ...
  let lastCheckpointSyncedBlock = previousLastSyncedBlock;
  ...
  const saveCheckpoint = (syncedThroughBlock, { force = false } = {}) => {
    ...
    if (!force && processedChunkCount < HISTORY_DOWNLOAD_CHECKPOINT_CHUNKS) {
      return;
    }
    if (!force && lastCheckpointSyncedBlock !== null && nextSyncedBlock <= lastCheckpointSyncedBlock) {
    ...
    lastCheckpointSyncedBlock = nextSyncedBlock;
    processedChunkCount = 0;
  };

  if (effectiveFromBlock <= latestBlock) {
    for (const { startBlock, endBlock } of iterateHistoryBlockRanges(effectiveFromBlock, latestBlock, chunkSize)) {
  ...
  const statefulSync = await syncSavedStatefulHistoryGames(publicClient, workingGames, normalizedWallet, syncTimestamp);
