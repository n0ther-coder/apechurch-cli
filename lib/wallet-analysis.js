...
export async function downloadWalletHistory(
  ...
  const baseHistoryGames = rebuildHistory ? [] : history.games;
  ...
  const statefulSync = await syncSavedStatefulHistoryGames(publicClient, baseHistoryGames, normalizedWallet, syncTimestamp);
  const allSyncedGames = [...syncedGames, ...statefulSync.games];
  const diagnosticsByGameKey = combineDiagnosticsMaps(
    await diagnoseUnsyncedSupportedGames(publicClient, baseHistoryGames, allSyncedGames, syncTimestamp),
    statefulSync.diagnosticsByGameKey
  );
  const mergedGames = mergeDownloadedHistoryGames(baseHistoryGames, allSyncedGames, syncTimestamp, diagnosticsByGameKey);
  ...
  const newUniqueGames = allSyncedGames.filter((game) => !existingKeys.has(gameKey(game.contract, historyGameId(game)))).length;
