...
import { VIDEO_POKER_CONTRACT } from './stateful/video-poker/constants.js';
...
export function mergeDownloadedHistoryGames(existingGames, syncedGames, syncTimestamp) {
...
function buildHistoryStatsSummary(
  ...
  return {
    ...
    rtp: toPercentNumber(totalPayoutWei, totalWageredWei, 1),
...
export function summarizeHistoryGamesByGame(history) {
  ...
  for (const game of allGames) {
    const groupKey = String(game.game_key || game.game || resolveHistoryGameName(game.contract)).toLowerCase();
    if (!groups.has(groupKey)) {
      groups.set(groupKey, {
        game: game.game || resolveHistoryGameName(game.contract),
        game_key: game.game_key || groupKey,
        savedGames: [],
      });
    }
...
export async function downloadWalletHistory(
  ...
  const mergedGames = mergeDownloadedHistoryGames(history.games, syncedGames, syncTimestamp);
