 ...
 *   with the local ABIs, so locally-known entries remain minimal until a
 *   reliable on-chain fetch strategy is implemented.
 */
...
export const UNSUPPORTED_HISTORY_GAMES = [
  {
    ...
    reason: 'No indexed per-user settlement event is available in the local contract ABI.',
  },
  {
    key: 'video-poker',
    name: 'Video Poker',
...
function buildSettledGames(settlementLogs, transactionMetadata, blockTimestamps, walletAddress, syncTimestamp) {
  ...
  for (const log of settlementLogs) {
    ...
    games.push({
      ...
      game: game?.name || resolveHistoryGameName(log.address),
...
function resolveHistoryStatsIdentity(game) {
  const displayGame = game?.game || resolveHistoryGameName(game?.contract);
  ...
  const variantKey = String(
    (useResolvedVideoPokerVariant ? resolvedVariant.variantKey : null)
    || game?.variant_key
    || resolvedVariant.variantKey
    || baseGameKey
  ).trim().toLowerCase();
  const variantLabel = (useResolvedVideoPokerVariant ? resolvedVariant.variantLabel : null)
    || game?.variant_label
  ...
  return {
    ...
    rtp_config: game?.rtp_config || resolvedVariant.rtpConfig || game?.config || null,
  };
}
...
export async function downloadWalletHistory(
  ...
  const diagnosticsByGameKey = await diagnoseUnsyncedSupportedGames(publicClient, history.games, syncedGames, syncTimestamp);
  const mergedGames = mergeDownloadedHistoryGames(history.games, syncedGames, syncTimestamp, diagnosticsByGameKey);
  ...
  const newUniqueGames = syncedGames.filter((game) => !existingKeys.has(gameKey(game.contract, game.gameId))).length;
