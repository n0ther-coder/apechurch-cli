...
function getSavedHistoryRewardTxHash(game) {
  const txHash = String(game?.tx || game?.settlement_tx || '').trim();
  return /^0x[a-fA-F0-9]{64}$/.test(txHash) ? txHash : null;
}
...
export async function syncSavedStatefulHistoryGames(publicClient, existingGames, walletAddress, syncTimestamp) {
  ...
  const statefulGames = (Array.isArray(existingGames) ? existingGames : []).filter((game) => {
    ...
    return contract === VIDEO_POKER_CONTRACT.toLowerCase()
      ...
      || contract === HI_LO_NEBULA_CONTRACT.toLowerCase();
  });
...
function applyStatefulReceiptTransfers(game, existingGame, transactionMetadataByHash, walletAddress) {
  ...
  const { gpReceivedRaw, wapeReceivedWei } = decodeReceiptTokenTransfers(txMeta.receipt, walletAddress);
  const receiptHasRewards = gpReceivedRaw > 0n || wapeReceivedWei > 0n;
  ...
  const txFrom = String(txMeta.tx?.from || '').toLowerCase();
  const normalizedWallet = String(walletAddress || '').toLowerCase();
  ...
  return {
    ...
    sponsored_transaction: txFrom ? txFrom !== normalizedWallet : (existingGame?.sponsored_transaction ?? game?.sponsored_transaction ?? false),
...
function resolveHistoryStatsIdentity(game) {
  ...
  const useResolvedRouletteVariant = resolvedVariant.gameKey === 'roulette'
    && Boolean(resolvedVariant.variantKey)
    && resolvedVariant.variantKey !== 'roulette';
  const useResolvedCanonicalVariant = useResolvedVideoPokerVariant || useResolvedPlinkoVariant || useResolvedRouletteVariant;
