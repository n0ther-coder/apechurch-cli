...
async function collectSettlementLogsForRange(publicClient, walletAddress, fromBlock, toBlock) {
  const endedLogs = [];
  const endedRangeLogs = await publicClient.getLogs({
  ...
  const fallbackRangeLogs = await publicClient.getLogs({
...
async function scanAdaptiveHistoryRange(
  ...
  try {
    ...
    if (startBlock >= endBlock) {
      throw new Error(
        `RPC log response is too large for block ${startBlock.toString()}; the range cannot be split further.`,
        { cause: error }
      );
    }
  }
  ...
  await scanAdaptiveHistoryRange(publicClient, walletAddress, startBlock, midpoint, onRange);
  await scanAdaptiveHistoryRange(publicClient, walletAddress, midpoint + 1n, endBlock, onRange);
}
...
async function scanSettlementLogRanges(
  ...
  for (const { startBlock, endBlock } of iterateHistoryBlockRanges(fromBlock, toBlock, initialMaxRange)) {
    await scanAdaptiveHistoryRange(publicClient, walletAddress, startBlock, endBlock, onRange);
