...
import { FORCE_CHIME_ENV_VAR, SUPPRESS_CHIME_ENV_VAR } from './constants.js';
...
export function canEmitWinChime({ isJson = false } = {}) {
  if (process.env[SUPPRESS_CHIME_ENV_VAR] === '1') {
    return false;
  }

  return process.env[FORCE_CHIME_ENV_VAR] === '1' || !isJson;
}
...
function canEmitBellFallback(stream = process.stderr, { allowNonTty = false } = {}) {
  return Boolean((allowNonTty || stream?.isTTY) && typeof stream?.write === 'function');
}
...
function queueTerminalBellFallback(beepCount, stream = process.stderr, options = {}) {
  if (!canEmitBellFallback(stream, options)) {
    return 0;
  }
  ...
  if (options.immediate) {
    try {
      stream.write(SLOT_BELL.repeat(intervals.length));
    } catch {
      return 0;
    }
...
export function queueWinChime(beepCount, { isJson = false, stream = process.stderr } = {}) {
  ...
  try {
    if (isJson && process.env[FORCE_CHIME_ENV_VAR] === '1' && !stream?.isTTY) {
      return queueTerminalBellFallback(count, stream, { allowNonTty: true, immediate: true });
    }

    if (process.stdout?.isTTY !== true && stream?.isTTY) {
