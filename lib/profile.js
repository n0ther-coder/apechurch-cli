...
import { saveScoresFromHistory } from './scores.js';
...
const DERIVED_HISTORY_CONFIG_FIELDS = new Set([
  'modeName',
  'riskName',
  'difficultyName',
  'winChance',
...
function stripPersistedHistoryGame(game) {
  ...
  persisted.config = sanitizePersistedHistoryConfig(persisted.config);
  return persisted;
}

function normalizeHistoryTimestampMs(game) {
...
function normalizeHistoryObjectField(value) {
  return value && typeof value === 'object' && !Array.isArray(value) ? value : null;
}

function sanitizePersistedHistoryConfig(config) {
  const rawConfig = normalizeHistoryObjectField(config);
  if (!rawConfig) {
    return null;
  }
  ...
  for (const [key, value] of Object.entries(rawConfig)) {
    if (DERIVED_HISTORY_CONFIG_FIELDS.has(key) || value === undefined || value === null) {
      continue;
    }
    sanitized[key] = value;
  }
...
function normalizeHistoryTextField(value) {
...
function normalizeHistoryGame(game) {
  ...
  const playTx = normalizeHistoryTextField(game?.play_tx) || normalizeHistoryTextField(game?.tx);
  const settlementTx = normalizeHistoryTextField(game?.settlement_tx) || null;
  const config = sanitizePersistedHistoryConfig(
    normalizeHistoryObjectField(game?.rtp_config) || normalizeHistoryObjectField(game?.config)
  );
  const normalized = {
    ...(game || {}),
    ...
    game_id: gameId,
    gameId,
    timestamp: normalizeHistoryTimestampMs(game),
    play_tx: playTx,
    settlement_tx: settlementTx,
    game_key: normalizeHistoryTextField(game?.game_key),
    config,
    transaction_from: normalizeHistoryTextField(game?.transaction_from),
  ...
  const hasWagerWei = hasRawHistoryValue(normalized.wager_wei);
  const hasPayoutWei = hasRawHistoryValue(normalized.payout_wei);
  if (hasWagerWei) {
    ...
    normalized.wape_received_wei = wagerWei.toString();
    normalized.wape_received_ape = formatEther(wagerWei);
...
export function saveHistory(history, walletAddress) {
  ...
  const persistedHistory = {
    version: HISTORY_SCHEMA_VERSION,
    wallet: normalizedHistory.wallet,
    chain_id: normalizedHistory.chain_id,
    last_synced_block: normalizedHistory.last_synced_block ?? null,
    last_download_on: normalizedHistory.last_download_on ?? null,
  ...
  saveScoresFromHistory(normalizedHistory, normalizedWallet, {
 ...
 * @param {Object} [game.config] - Canonical saved play configuration
...
export function saveGameToHistory({
  ...
  settled = undefined,
  wager_wei = undefined,
  payout_wei = undefined,
  contract_fee_wei = undefined,
  gas_fee_wei = undefined,
  gp_received_raw = undefined,
  gp_source = undefined,
  last_sync_on = undefined,
  last_sync_msg = undefined,
  walletAddress,
}) {
  const normalizedWallet = normalizeHistoryWalletAddress(walletAddress);
  if (!normalizedWallet) {
  ...
  const existingIndex = history.games.findIndex((entry) =>
    ...
    && String(entry.game_id ?? entry.gameId ?? '') === normalizedGameId
  );
  const existingEntry = existingIndex >= 0 ? history.games[existingIndex] : {};
  const resolveRawField = (field, value) => (
    value !== undefined && value !== null
      ? String(value)
      : existingEntry[field] ?? undefined
  );
  const nextEntry = {
    ...existingEntry,
    ...
    timestamp: existingIndex >= 0 && Number(history.games[existingIndex].timestamp || 0) > 0
      ? history.games[existingIndex].timestamp
      : normalizedTimestamp,
    play_tx: existingIndex >= 0
      ? history.games[existingIndex].play_tx ?? history.games[existingIndex].tx ?? (typeof tx === 'string' ? tx : null)
      : (typeof tx === 'string' ? tx : null),
    settlement_tx: existingIndex >= 0 && typeof tx === 'string'
      ? tx
      : (existingIndex >= 0 ? history.games[existingIndex].settlement_tx ?? null : null),
    ...
    settled: settled !== undefined ? Boolean(settled) : existingEntry.settled,
    wager_wei: resolveRawField('wager_wei', wager_wei),
    payout_wei: resolveRawField('payout_wei', payout_wei),
    contract_fee_wei: resolveRawField('contract_fee_wei', contract_fee_wei),
    gas_fee_wei: resolveRawField('gas_fee_wei', gas_fee_wei),
    ...
    last_sync_on: typeof last_sync_on === 'string'
      ? last_sync_on
      : (existingIndex >= 0 ? history.games[existingIndex].last_sync_on ?? null : null),
    last_sync_msg: typeof last_sync_msg === 'string'
      ? last_sync_msg
      : (existingIndex >= 0 ? history.games[existingIndex].last_sync_msg ?? null : null),
  };
