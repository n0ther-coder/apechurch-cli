...
export function formatLoopProgress({ currentBalanceApe, startingBalanceApe, stats, nextDelayLabel }) {
  ...
  const points = totalWageredApe * 10;
  ...
  const pointsLine = netResultApe > 0
    ...
    : `🧮 Points: ${formatPoints(points)}, ${netResultApe !== 0 ? (points / Math.abs(netResultApe)).toFixed(1) : 'n/a'} GB/APE`;

  return [
    `💰 Balance: ${Number(currentBalanceApe).toFixed(2)} APE (${formatSignedApe(balanceDeltaApe)}) | ✌️ Win rate: ${winRate.toFixed(1)}% (${wins}/${completedGames}) | 🎲 RTP: ${rtp.toFixed(1)}% (payout ${totalPayoutApe.toFixed(2)}  wagered ${totalWageredApe.toFixed(2)})`,
    pointsLine,
