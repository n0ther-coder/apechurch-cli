 ...
 */
import { GP_PER_APE } from './constants.js';
...
export function formatBalanceSnapshot({
  label = 'Balance',
  currentBalanceApe,
  startingBalanceApe = null,
} = {}) {
  const currentBalance = Number(currentBalanceApe) || 0;
  const baseLine = `⚖️  ${label}: ${currentBalance.toFixed(2)} APE`;

  if (startingBalanceApe === null || startingBalanceApe === undefined) {
    return baseLine;
  }

  const startingBalance = Number(startingBalanceApe) || 0;
  return `${baseLine} (${formatSignedApe(currentBalance - startingBalance)})`;
}
...
function formatPointsLine({ points, netResultApe }) {
  if (netResultApe > 0) {
    return `🧮 Points: ${formatPoints(points)} (+${formatPoints(points)} GP, +${netResultApe.toFixed(2)} APE)`;
  }
  if (netResultApe === 0) {
    return `🧮 Points: ${formatPoints(points)} (+${formatPoints(points)} GP)`;
  }
  return `🧮 Points: ${formatPoints(points)} (${(points / Math.abs(netResultApe)).toFixed(1)} GP/APE)`;
}

function getDerivedStats(stats = {}, { gpPerApe = GP_PER_APE } = {}) {
  ...
  const points = totalWageredApe * gpPerApe;
...
function formatLoopProgressBar(currentGame, totalGames, width = 60) {
  if (
    !Number.isFinite(currentGame)
    || currentGame <= 0
  ...
  const normalizedRatio = Math.min(Math.max(currentGame / totalGames, 0), 1);
  const filledWidth = Math.round(normalizedRatio * width);
...
export function formatLoopGameCompletion({ currentGame, maxGames = null, gameId = null } = {}) {
  if (!Number.isFinite(currentGame) || currentGame <= 0) {
    return '';
  }

  const lines = [
    `Game ${theme.bold(String(currentGame))} complete${gameId ? ` ${theme.gray(`(${gameId})`)}` : ''}`,
  ];

  if (Number.isFinite(maxGames) && maxGames > 0) {
    lines.push(formatLoopProgressBar(currentGame, maxGames));
  }

  return lines.join('\n');
}

export function formatLoopProgress({ currentBalanceApe, startingBalanceApe, stats, nextDelayLabel, rtpGame = null, rtpConfig = null, gpPerApe = GP_PER_APE }) {
  const {
    completedGames,
    wins,
    winRate,
    points,
    netResultApe,
  } = getDerivedStats(stats, { gpPerApe });
  const scopedRtpStats = getDerivedStats(getScopedLoopStats(stats, { rtpGame, rtpConfig }), { gpPerApe });
...
export function formatSessionStats({ gamesPlayed, startingBalanceApe, endingBalanceApe, stats, rtpGame = null, rtpConfig = null, gpPerApe = GP_PER_APE } = {}) {
  const startingBalance = Number(startingBalanceApe) || 0;
