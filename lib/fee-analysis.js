...
export const DEFAULT_FEE_ANALYSIS_OUTLIER_FEE_BPS = 1_000;
...
function expandAggregate(aggregate) {
  ...
  const costWei = parseBigIntField(normalized.fw) + parseBigIntField(normalized.gw);
  return {
    games: normalized.n,
    wins: normalized.w,
    pushes: normalized.p,
    ...
    avg_wager_wei: normalized.n > 0 ? (parseBigIntField(normalized.bw) / BigInt(normalized.n)).toString() : '0',
    avg_wager_ape: normalized.n > 0 ? toApe(parseBigIntField(normalized.bw) / BigInt(normalized.n)) : '0',
    ...
    avg_cost_wei: normalized.n > 0 ? (costWei / BigInt(normalized.n)).toString() : '0',
    avg_cost_ape: normalized.n > 0 ? toApe(costWei / BigInt(normalized.n)) : '0',
    avg_cost_bps: ratioBpsNumber(costWei, normalized.bw),
...
function buildFeeOutliers(extremes, { feeBpsThreshold = DEFAULT_FEE_ANALYSIS_OUTLIER_FEE_BPS } = {}) {
  const outliers = [];
  const minFee = extremes.min_fee;
  if (minFee && parseBigIntField(minFee.value) === 0n && parseBigIntField(minFee.wager_wei) > 0n) {
    outliers.push({
      type: 'zero_fee',
      severity: 'warning',
      description: 'Observed zero fee on a settled game with positive wager.',
      extreme: minFee,
    });
  }

  const maxFeeBps = extremes.max_fee_bps;
  const maxFeeBpsValue = maxFeeBps?.value === null || maxFeeBps?.value === undefined
    ? null
    : Number(maxFeeBps.value);
  if (maxFeeBps && Number.isFinite(maxFeeBpsValue) && maxFeeBpsValue > feeBpsThreshold) {
    outliers.push({
      type: 'high_fee_bps',
      severity: 'warning',
      description: 'Observed fee/wager ratio above the configured threshold.',
      threshold_bps: feeBpsThreshold,
      value_bps: maxFeeBpsValue,
      extreme: maxFeeBps,
    });
  }
...
export function buildFeeReport(gameInput, {
  ...
  return {
    ...
    extremes,
    outliers: buildFeeOutliers(extremes),
  };
}
