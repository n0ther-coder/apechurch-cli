...
import { BOT_PLAIN_OUTPUT_ENV_VAR } from '../constants.js';
import {
...
export function colorBotCommand(value, colorOutput) {
  return colorOutput ? chalk.yellow.dim(value) : value;
}

export function colorValue(value, colorOutput) {
  return colorOutput ? chalk.white(value) : value;
}
...
export function shouldUsePlainColorOutput({ json = false } = {}) {
  return Boolean(resolvePlainOutputStream({ json })?.isTTY);
}
...
function colorPayout(value, payoutWei, wagerWei, colorOutput) {
  if (!colorOutput) return value;
  if (payoutWei > wagerWei) return chalk.green(value);
  if (payoutWei === wagerWei) return chalk.magenta(value);
...
export function formatPlayCommandSuffix(payload, { colorOutput = false } = {}) {
  ...
  const betText = formatWeiAsApeRounded(amounts.wagerWei);
  const payoutText = formatWeiAsApeRounded(amounts.payoutWei);
...
export function formatIterationSummaryLine({
  ...
  const fields = [
    `${colorKey('game_n', colorOutput)} ${colorValue(String(gameNumber), colorOutput)}`,
