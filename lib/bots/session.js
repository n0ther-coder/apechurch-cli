...
export function getStandardBotLoopCondition({
  ...
  if (recoverLossWei !== null && loopState.minPnlWei < 0n) {
    const recoveredWei = trailingPnlWei - loopState.minPnlWei;
    if (recoveredWei >= recoverLossWei) {
      return {
        kind: 'recover_loss',
  ...
  if (givebackProfitWei !== null && loopState.maxPnlWei > 0n) {
    const givebackWei = loopState.maxPnlWei - trailingPnlWei;
    if (givebackWei >= givebackProfitWei) {
      return {
