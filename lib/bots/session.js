...
export function parseStandardBotArgs(args = []) {
  ...
  for (let i = 0; i < tokens.length;) {
    ...
    if (apeOption) {
      ...
      const amount = parsePositiveApeAmount(parsed.value);
      if (!amount) {
        throw new Error(`Invalid ${apeOption} value: "${parsed.value}". Must be a positive APE amount.`);
      }
    ...
    const booleanOption = [...STANDARD_BOT_LOOP_BOOLEAN_OPTIONS.keys()]
      .find((name) => token === name || token.startsWith(`${name}=`));
    if (booleanOption) {
      const parsed = readBooleanOption(tokens, i, booleanOption);
...
export async function prepareStandardBotLoopRuntime({
  ...
  if (!hasStandardBotLossGuard(controls)) {
    await confirmStandardBotWarning(
      `${botName} was invoked without --stop-loss or --max-loss; it could drain all available funds.`,
      { json },
    );
  }

  if (!hasStandardBotProfitGuard(controls)) {
    await confirmStandardBotWarning(
  ...
  const profitGuard = controls.takeProfit ?? controls.minProfit;
  if (profitGuard !== null) {
    runtime.cliTakeProfitApe = formatApeNumber(startingBalanceApe + Number(profitGuard));
  }

  const lossGuard = controls.stopLoss ?? controls.maxLoss;
  if (lossGuard !== null) {
    runtime.cliStopLossApe = formatApeNumber(Math.max(startingBalanceApe - Number(lossGuard), 0));
  }
...
export function getStandardBotNestedBotForwardTokens(loopControls = {}, {
  ...
  for (const [optionName, key] of STANDARD_BOT_LOOP_APE_OPTIONS.entries()) {
    if (controls[key] !== null) {
      tokens.push(optionName, controls[key]);
    }
  }
...
export function getStandardBotLoopCondition({
  ...
  if (takeProfitWei !== null && pnlWei >= takeProfitWei) {
    return { kind: 'take_profit', threshold_ape: controls.takeProfit, pnl_ape: formatWeiAsApe(pnlWei), executions };
  }
  ...
  if (stopLossWei !== null && pnlWei <= -stopLossWei) {
    return { kind: 'stop_loss', threshold_ape: controls.stopLoss, pnl_ape: formatWeiAsApe(pnlWei), executions };
  }
