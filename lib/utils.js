...
function collectErrorMessages(error, seen = new Set()) {
  ...
  if (Array.isArray(error.metaMessages)) {
    messages.push(...error.metaMessages.filter((value) => typeof value === 'string' && value.trim()));
  }

  for (const field of ['cause', 'error', 'data']) {
    if (error[field] && typeof error[field] === 'object') {
      messages.push(...collectErrorMessages(error[field], seen));
    }
  }
...
function isGenericViemMessage(message) {
  return /unknown error occurred while executing the contract function/i.test(message)
    || /execution reverted for an unknown reason/i.test(message);
}
...
export function sanitizeError(error) {
  ...
  const msg = messages.find((message) => !isGenericViemMessage(message))
    || messages[0]
    || String(error);
  const allMessages = [...new Set([msg, ...messages])].join('\n');
  const lower = allMessages.toLowerCase();

  // --- RPC/Network Errors ---
  if (lower.includes('could not coalesce') || lower.includes('failed to fetch')) {
  ...
  if (lower.includes('execution reverted')) {
    ...
    const revertMessage = messages.find((message) => /execution reverted/i.test(message) && !isGenericViemMessage(message))
      || messages.find((message) => /execution reverted/i.test(message))
      || allMessages;
    const match = revertMessage.match(/execution reverted[:\s]*(.+?)(?:\n|$)/i);
    const reason = match?.[1]?.trim();
    return reason && !/for an unknown reason/i.test(reason)
...
export function clampRange(min, max, low, high) {
  ...
  const clampedMin = Math.min(Math.max(normalizedMin, low), high);
  const clampedMax = Math.max(Math.min(normalizedMax, high), low);
