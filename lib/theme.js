...
export function formatHistoryLine(game) {
  ...
  const gameName = theme.gameName(game.game.padEnd(16));
  const wager = theme.dim(`${parseFloat(game.wager_ape).toFixed(2)} APE →`);
  const pnl = formatPnL(game.pnl_ape, 2);

  return `   ${status} ${gameName} ${wager} ${pnl}`;
}
