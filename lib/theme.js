...
export function colorBox(lines, opts = {}) {
  ...
  for (const line of lines) {
    // Note: line may contain ANSI codes, so padding is approximate
    // For accurate padding, strip ANSI, pad, then apply colors separately
    const paddedLine = line.padEnd(innerWidth);
    output.push(borderColor('║') + paddedLine.slice(0, innerWidth) + borderColor('║'));
  }
 ...
 */
export function formatHistoryLine(game) {
  const pnl = parseFloat(game.pnl_ape);
  const hasEconomicData = Number.isFinite(pnl) && Number.isFinite(parseFloat(game.wager_ape));
  const status = hasEconomicData ? formatOutcomeIcon(pnl, game.settled) : theme.pending('🕓');
  const gameIdLabel = `<${String(game.gameId || 'unknown')}>`;
  const rawTimestamp = Number(game.chain_timestamp || game.timestamp || 0);
  const timestampMs = rawTimestamp > 0 && rawTimestamp < 1e12 ? rawTimestamp * 1000 : rawTimestamp;
  const timestampLabel = Number.isFinite(timestampMs) && timestampMs > 0
    ? new Date(timestampMs).toISOString().replace(/\.\d{3}Z$/, ' UTC').replace('T', ' ')
    : 'unknown UTC';
  const formatHistoryResultAmount = (apeAmount) => {
  ...
  const netResultLabel = !hasEconomicData
    ? theme.pending(' on-chain sync pending')
    : !game.settled
    ? theme.pending('pending')
    : pnl > 0
      ? theme.positive(formatHistoryResultAmount(pnl))
      : pnl < 0
        ? theme.negative(formatHistoryResultAmount(pnl))
        : theme.zero(formatHistoryResultAmount(0));
  const wagerLabel = hasEconomicData
    ? theme.amount(`(wagered ${parseFloat(game.wager_ape).toFixed(2)} APE)`)
    : theme.dim(`(${game.last_sync_msg || 'awaiting sync'})`);

  return `   ${theme.dim(timestampLabel)} ${status} ${netResultLabel} ${wagerLabel} ${theme.gameName(game.game)} ${theme.dim(gameIdLabel)}`;
}
