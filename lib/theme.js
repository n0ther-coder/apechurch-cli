...
export function formatResult(won, pnl, settled = true) {
  ...
  if (num === 0) {
    return theme.push(`🔄 ${formatPnL(pnl)}`);
  }

  const icon = won ? '✅' : '❌';
  return `${icon} ${formatPnL(pnl)}`;
}
...
export function formatHistoryLine(game) {
  ...
  const netResultLabel = !game.settled
  ...
  return `   ${theme.dim(timestampLabel)} ${status} ${netResultLabel} ${theme.amount(`(wagered ${parseFloat(game.wager_ape).toFixed(2)} APE)`)} ${theme.gameName(game.game)} ${theme.dim(gameIdLabel)}`;
}
