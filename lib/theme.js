...
import { FORCE_COLOR_ENV_VAR } from './constants.js';
...
export const theme = {
  ...
  rtpAbove: chalk.greenBright.bold,
  rtpBelow: chalk.redBright.bold,
...
export function formatHistoryLine(game, opts = {}) {
  ...
  const gpValue = Number(game.gp_received_display ?? game.gp_received_raw ?? 0);
  const gpLabel = Number.isFinite(gpValue) && gpValue > 0
    ? theme.dim(`🧮 ${formatHistoryGpAmount(game.gp_received_display ?? game.gp_received_raw)} GP`)
    : null;
  ...
  const timestampLabel = formatTerminalTimestampLabel(game.chain_timestamp || game.timestamp || 0);
  const gameLabel = fitAnsiText(theme.gameName(game.game), 16);
  const sourceTimestampLabel = formatTerminalTimestampLabel(game.last_sync_on);
  ...
  const sourceLabel = hasEconomicData
    ? sourceTimestampLabel !== 'unknown timestamp'
...
function formatHistoryGpAmount(gpAmount) {
  const numeric = Number(gpAmount);
  if (Number.isFinite(numeric)) {
    return Number.isInteger(numeric) ? numeric.toString() : numeric.toFixed(1).replace(/\.0$/, '');
  }

  return String(gpAmount || '0');
}
...
function formatTerminalTimestampLabel(rawValue) {
  ...
  if (typeof rawValue === 'string' && Number.isNaN(Number(rawValue))) {
    ...
    return Number.isFinite(parsed) && parsed > 0
      ? formatTerminalTimestamp(parsed)
      : 'unknown timestamp';
  }
  ...
  return formatTerminalTimestamp(timestampMs) || 'unknown timestamp';
}
...
/**
 * Force ANSI color output for plain, non-JSON CLI rendering.
 *
 * Chalk detects color support at import time, so the CLI calls this after
 * parsing global process flags/env when users explicitly request --color.
 */
export function forceColorOutput() {
  process.env[FORCE_COLOR_ENV_VAR] = '1';
  if (chalk.level < 1) {
    chalk.level = 1;
  }
}
