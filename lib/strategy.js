 ...
 * - slots: { spins: [min, max] }
 * - blocks: { mode: [min, max], runs: [min, max] }; grid remains the implicit 3x3 default unless --grid is passed
 *
...
export function applyProfileOverrides(strategyConfig, overrides) {
  ...
  if (overrides.gimbozSmash) {
    nextConfig.gimbozSmash = {
      ...nextConfig.gimbozSmash,
      winCount: normalizeRange(overrides.gimbozSmash.winCount, nextConfig.gimbozSmash?.winCount || [40, 60]),
    };
  }
 ...
 * // Returns: { game: 'jungle-plinko', mode: 2, split: 75 }
...
export function selectGameAndConfig(strategyConfig) {
  ...
  // Parameters: mode (risk level), split (number of independent drops)
  // -------------------------------------------------------------------------
  if (gameEntry.type === 'plinko') {
    ...
    const split = randomIntInclusive(ballMin, ballMax);

    return { game: gameEntry.key, mode, split };
  }

  // -------------------------------------------------------------------------
  // SLOTS-TYPE GAMES (Dino Dough, Bubblegum Heist)
  // Parameters: split (number of slot pulls)
  // -------------------------------------------------------------------------
  if (gameEntry.type === 'slots') {
    ...
    const split = randomIntInclusive(spinMin, spinMax);

    return { game: gameEntry.key, split };
  }

  // -------------------------------------------------------------------------
  // BLOCKS
  // Parameters: grid (implicit 3x3 default), mode (0-1), survive (1-5).
  // Automatic selection keeps the legacy compounding behavior; independent
  // Blocks rolls are requested explicitly with --split.
  // -------------------------------------------------------------------------
  if (gameEntry.type === 'blocks') {
    ...
    const mode = randomIntInclusive(modeMin, modeMax);
    const survive = randomIntInclusive(runsMin, runsMax);
    const gridMode = Number(gameEntry.config.grid?.default ?? 0);

    return { game: gameEntry.key, grid: '3x3', gridMode, mode, survive };
  }
  ...
  // -------------------------------------------------------------------------
  // PRIMES
  // Parameters: difficulty (0-3), split (1-20)
  // -------------------------------------------------------------------------
  if (gameEntry.type === 'primes') {
    ...
    const split = randomIntInclusive(runsMin, runsMax);

    return { game: gameEntry.key, difficulty, split };
  }
